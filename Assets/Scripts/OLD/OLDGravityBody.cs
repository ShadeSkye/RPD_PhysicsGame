...
public class OLDGravityBody : MonoBehaviour
{
    ...
    [SerializeField] public OLDGravityBody orbitTarget;
    ...
    private void Awake()
    {
        ...
        if (isGravitySource) // generate mas from gravity and radius
        {
            ...
            rb.mass = (localGravity * radius * radius) / OLDGravityManager.Instance.gravitationalConstant;
            transform.localScale = Vector3.one * radius * 2;
        }
        else if (TryGetComponent<OLDCargo>(out OLDCargo cargo))
        {
            rb.mass = Mathf.Clamp(cargo.weight, 1f, 15f);
            bodyName = cargo.objectName;
        }
    }
    ...
    private void LateUpdate()
    {
        ...
        OLDGravityManager.Instance.RegisterBody(this);
        isInitialised = true;
    }
    ...
    private void CalculateInitialVelocity()
    {
        ...
        // calculate velocity
        float velocityMagnitude = Mathf.Sqrt(OLDGravityManager.Instance.gravitationalConstant * orbitTarget.rb.mass / orbitalRadius);
    ...
    private Vector3 CalculateOrbitalPlane()
    {
        ...
        if (r <= OLDGravityManager.Instance.alignedOrbitChance) // chance of being normal
        {
            return  Vector3.up;
        }
        else
    ...
    {
        if (OLDGravityManager.Instance != null)
            OLDGravityManager.Instance.UnregisterBody(this);
    }
