...
public class OLDGravityManager : MonoBehaviour
{
    public static OLDGravityManager Instance { get; private set; }

    [Header("Gravity Settings")]
    public float gravitationalConstant = 0.001f;

    private List<OLDGravityBody> bodies = new List<OLDGravityBody>();
    public List<OLDGravityBody> GetBodies() => bodies;
    ...
    public void RegisterBody(OLDGravityBody body)
    {
        if (!bodies.Contains(body))
            bodies.Add(body);
    }

    public void UnregisterBody(OLDGravityBody body)
    {
        bodies.Remove(body);
    }
    ...
    private void ApplyGravity()
    {
        for (int x = 0; x < bodies.Count; x++) // for each body
        {
            ...
            {
                OLDGravityBody strongest = GetStrongestGravitySource(a);
                if (strongest != null)
                {
    ...
    private Vector3 CalculateGravity(OLDGravityBody a, OLDGravityBody b)
    {
    ...
    private OLDGravityBody GetStrongestGravitySource(OLDGravityBody a)
    {
        OLDGravityBody strongestBody = null;
        float strongestForce = 0;

        foreach (OLDGravityBody body in bodies)
        {
